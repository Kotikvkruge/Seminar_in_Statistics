...
# plot points on it
ggmap(reg_basemap) +
  geom_point(data = df, aes(x, y), color = "dark orange",  size = 1.5) # Figure 3.XX
...
obj_coord <- rbind(obj_coord, c('Waste', 12.13134276302641, 49.0616639512157)) #add missing waste burning location 
...
# distances between objects using the script '' in Python.
...
ggmap(reg_basemap) +
  geom_point(data = df[c(1, 200),], aes(x, y), color = "dark orange",  size = 2) # Figure 3.XX
...
for (t in c(1:length(obj_types))) {               # Select a distance from a rental unit to
  for (i in c(1: (ru_max_ind-ru_min_ind + 1) )) { # its nearest object for each object type
    vec.temp <- driving_dist[ (ru_min_ind -1 + i), which(driving_dist[, ru_max_ind+1] == obj_types[t]) ]
...
for (t in c(1:length(obj_types))) {               # Select a distance from a rental unit to
  for (i in c(1: (ru_max_ind-ru_min_ind + 1) )) { # its nearest object for each object type
    vec.temp <- driving_time[(ru_min_ind -1 + i), which(driving_time[, ru_max_ind+1] == obj_types[t]) ]
    if (length(vec.temp) == 1) { # for single objects within. type, e.g. Uni or Center
...
# In this section three regression models: Ordinaty Least Squares (OLS),
# Generalized Least Squares (GLS) and Quantile  Regression (QR)
...
ols_dd <- lm(rpsqm ~ x + y + size + age + city  + bath   # Property features
...
ols_dt <- lm(rpsqm ~ x + y + size + age + city  + bath   # Property features
            + SB1 + Center + Hospital + School + Playground + Waste + Harbor # Location features
            + Park + Uni + Industry,       #
            data = df_driving_time_st)
...
ols_ed <- lm(rpsqm ~ x + y + size + age + city  + bath   # Property features
            + SB1 + Center + Hospital + School + Playground + Waste + Harbor # Location features
            + Park + Uni + Industry,       #
            data = df_eucl_dist_st)
...
ols_md <- lm(rpsqm ~ x + y + size + age + city  + bath   # Property features
         + SB1 + Center + Hospital + School + Playground + Waste + Harbor # Location features
         + Park + Uni + Industry,       #
         data = df_manh_dist_st)
...
# 
# formula_qr <- rpsqm ~ qss(size) + qss(age) + qss(cbind(x,y)) + qss(Center) + Hospital + School + Waste + Park + Harbor + Uni
# # Same logic as in case of GAM described above in 3.2)
# 
# ### 3.1 a) Driving distance
# 
# qr_dd <- rqss(formula_qr, tau = 0.5, 
#                           data = df_driving_dist_st)
# 
# summary(qr_dd)
# 
# ### 3.1 b) Driving time
# 
# qr_dt <- rqss(formula_qr, tau = 0.5, 
#               data = df_driving_time_st)
# summary(qr_dt)
# 
# ### 3.1 c) Euclidean distance
# 
# qr_ed <- rqss(formula_qr, tau = 0.5, 
#               data = df_eucl_dist_st)
# summary(qr_ed)
# 
# ### 3.1 d) Manhattan distance
# 
# qr_md <- rqss(formula_qr, tau = 0.5, 
#               data = df_manh_dist_st)
# summary(qr_md)
...
CV_res <- matrix('NA', nrow = 9, ncol = 7)
CV_res[1,] <- c('Method', 'Resubstitution', 'Hold-out', '10-Fold CV', 'LOO CV', 'Spatial blocking CV', 'Buffered CV')
CV_res[2:9,1] <- c( 'Driving distance GAM', 'Driving distance QR', 'Driving time GAM', 'Driving time QR','Euclidean distance GAM', 'Euclidean distance QR','Manhattan distance GAM', 'Manhattan distance QR')
CV_res
...
# CV_res[3,2] <- RMSE_0(qr_dd)

### 4.1 b) Driving time

RMSE_0(ols_dd)
CV_res[4,2] <- RMSE_0(gam_dt)
# CV_res[5,2] <- RMSE_0(qr_dt)

### 4.1 c) Euclidean distance

RMSE_0(ols_dd)
CV_res[6,2] <- RMSE_0(gam_ed)
# CV_res[7,2] <- RMSE_0(qr_ed)

### 4.1 d) Manhattan distance

RMSE_0(ols_dd)
CV_res[8,2] <- RMSE_0(gam_md)
# CV_res[9,2] <- RMSE_0(qr_md)

#### 4.2) Hold-out #### 

set.seed(42)
training.samples <- df_driving_dist_st$rpsqm %>%
  createDataPartition(p = 0.8, list = FALSE)

Hold_out_gam <- function(data){
...
# variables <- c('rpsqm', 'size', 'age', 'x', 'y', 'Center', 'Hospital', 'School', 'Waste', 'Park', 'Harbor', 'Uni')
# 
# Hold_out_qr <- function(data){
#   train <- data[training.samples, variables]
#   test <- data[-training.samples, variables]
#   model_ho <-  rqss(formula_qr,
#                     tau = 0.5,
#                     data = train)
#   
#   # assure that all points lay within the convex hull
#   xy.unique <- unique(train[ , c("x", "y")])
#   T <- with(data=xy.unique, expr=tri.mesh(x,y))
#   tri.m <- tri.mesh(x=T$x, y=T$y, duplicate="error")
#   xd <- test[ , c("x")]
#   yd <- test[ , c("y")]
#   con.idx <- in.convex.hull(tri.obj=T, x=xd, y=yd) & xd < max(xy.unique$x) & yd < max(xy.unique$y) & xd > min(xy.unique$x) & yd > min(xy.unique$y)
#   
#   # fit the QR
#   qr.fit.val <- predict.rqss(model_ho, newdata = test[con.idx,])
# 
#   RMSE_ho <- round(mean(sqrt((qr.fit.val - data[con.idx, 'rpsqm'])^2)), 3)
#   return(RMSE_ho)
# }
...
CV_res[2,3] <- Hold_out_gam(df_driving_dist_st)

# CV_res[3,3] <- Hold_out_qr(df_driving_dist_st)

### 4.2 b) Driving time

CV_res[4,3] <- Hold_out_gam(df_driving_time_st)

# CV_res[5,3] <- Hold_out_qr(df_driving_time_st)

### 4.2 c) Euclidean distance

CV_res[6,3] <- Hold_out_gam(df_eucl_dist_st)

# CV_res[7,3] <- Hold_out_qr(df_eucl_dist_st)

### 4.2 d) Manhattan distance

CV_res[8,3] <- Hold_out_gam(df_manh_dist_st)

# CV_res[9,3] <- Hold_out_qr(df_manh_dist_st)

#### 4.3) 10-Fold CV #### 

CV_K.Fold <- function(dats, n.folds = 10){
...
CV_res[2,4] <- CV_K.Fold(df_driving_dist_st)

# CV_res[3,4] <- Hold_out_qr(df_driving_dist_st)

### 4.3 b) Driving time

CV_res[4,4] <- CV_K.Fold(df_driving_time_st)

# CV_res[5,4] <- Hold_out_qr(df_driving_time_st)

### 4.3 c) Euclidean distance

CV_res[6,4] <- CV_K.Fold(df_eucl_dist_st)

# CV_res[7,4] <- Hold_out_qr(df_eucl_dist_st)

### 4.3 d) Manhattan distance

CV_res[8,4] <- CV_K.Fold(df_manh_dist_st)

# CV_res[9,4] <- Hold_out_qr(df_manh_dist_st)
...
CV_res[2,5] <- CV_K.Fold(df_driving_dist_st, K)

# CV_res[3,5] <- Hold_out_qr(df_driving_dist_st)

### 4.4 b) Driving time

CV_res[4,5] <- CV_K.Fold(df_driving_time_st, K)

# CV_res[5,5] <- Hold_out_qr(df_driving_time_st)

### 4.4 c) Euclidean distance

CV_res[6,5] <- CV_K.Fold(df_eucl_dist_st, K)

# CV_res[7,5] <- Hold_out_qr(df_eucl_dist_st)

### 4.4 d) Manhattan distance

CV_res[8,5] <- CV_K.Fold(df_manh_dist_st, K)

# CV_res[9,5] <- Hold_out_qr(df_manh_dist_st)


#### 4.5) CV with spatial blocking #### 

set.seed(42)
CV_sb_partition <- cv_spatial(spdf, k = 10, selection = "systematic")

CV_K.Fold_block <- function(dats, n.folds = 10){
...
CV_res[2,6] <- CV_K.Fold_block(spdf_driving_dist_st)

### 4.5 b) Driving time

CV_res[4,6] <- CV_K.Fold_block(spdf_driving_time_st)

### 4.5 c) Euclidean distance

CV_res[6,6] <- CV_K.Fold_block(spdf_eucl_dist_st)

### 4.5 d) Manhattan distance

CV_res[8,6] <- CV_K.Fold_block(spdf_manh_dist_st)


#### 4.6) LOO CV with spatial buffering #### 

#Define a function returning RMSE of LOO CV with spatial buffering
LOO_CV_buffer <- function(data, distances){
  results <- c()
...
CV_res[2,7] <- LOO_CV_buffer(df_driving_dist_st, driving_dist)

### 4.6 b) Driving time

CV_res[4,7] <- LOO_CV_buffer(df_driving_time_st, driving_time)

### 4.6 c) Euclidean distance

CV_res[6,7] <- LOO_CV_buffer(df_eucl_dist_st, eucl_dist)

### 4.6 d) Manhattan distance

CV_res[8,7] <- LOO_CV_buffer(df_manh_dist_st, manh_dist)

write.csv(CV_res, file = 'CV Results.csv')
...
S.dist  <-  dnearneigh(coo, 0, quantile(dist(coo), 0.1)) # Define properties within .1 quantile distance as neighboring

lw <- nb2listw(S.dist, style="W",zero.policy=T) # define search circle

set.seed(42) # Reproducible results
MI  <-  moran.mc(df_manh_dist_st$rpsqm, lw, nsim = 1000, zero.policy=T) # Run the MC simulation

plot(density(MI$res), main="") # plots the results
abline(v = MI$statistic, col = 'red') # Figure 3.XX

MI # p-value ≈ 0.007 => reject H0: zero spatial autocorrelation present in rents at \alpha=1\%
...
abline(h = mean(MI_stats), col = 'gray') # Figure 3.XX


# 5.2) Variogramm ####

plot(variogram(rpsqm~1, spdf), type = "b", main = "Variogram: ")

vario <- variogram(residuals(gam_dd, 'normalized') ~ 1, data = spdf_driving_dist_st)
plot(vario$dist, vario$gamma, type = "b"cutoff = 6, alpha = c(0,
                                                              45, 90, 135)))
plot(gam_dd)
