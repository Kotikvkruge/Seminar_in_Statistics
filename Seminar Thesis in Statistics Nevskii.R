...
Hold_out_rmse <- function(data){
  ...
  RMSE_ho <- round(sqrt (
    mean((predict(model_ho, data[-training.samples, ]) 
...
CV_K.Fold_rmse <- function(dats, n.folds = 10){
  ...
  return(round(mean(results_rmse), 3))
}
...
CV_K.Fold_mae <- function(dats, n.folds = 10){
  ...
  return(round(mean(results_mae), 3))
}
...
LOO_rmse <- function(dats){
  results_rmse <- c()
  for (i in 1:dim(dats)[1]){
    train <- dats[-i, ] #split into train and test sets
    test <- dats[i, ]
    gam.model <- gam(formula_gam, data = train)
    pred <- predict(gam.model, newdata = test)
    error <- test$rpsqm - pred
    results_rmse[i]<- error
  }
  return( round(sqrt(mean(results_rmse^2)), 3) )
}

LOO_mae <- function(dats){
    results_mae <- c()
    for (i in 1:dim(dats)[1]){
      train <- dats[-i, ] #split into train and test sets
      test <- dats[i, ]
  ...
  return( round(mean(abs(results_mae)), 3) )
}

### 4.4 a) Driving distance

CV_res[2,5] <- LOO_rmse(df_driving_dist_st)

CV_res[3,5] <- LOO_mae(df_driving_dist_st)

### 4.4 b) Driving time

CV_res[4,5] <- LOO_rmse(df_driving_time_st)

CV_res[5,5] <- LOO_mae(df_driving_time_st)

### 4.4 c) Euclidean distance

CV_res[6,5] <- LOO_rmse(df_eucl_dist_st)

CV_res[7,5] <- LOO_mae(df_eucl_dist_st)

### 4.4 d) Manhattan distance

CV_res[8,5] <- LOO_rmse(df_manh_dist_st)

CV_res[9,5] <- LOO_mae(df_manh_dist_st)
...
CV_res_df <- as.data.frame(CV_res[2:9, 2:8], row.names = CV_res[2:9, 1])
colnames(CV_res_df) <- CV_res[1, 2:8]

write.csv(CV_res_df, file = 'CV Results.csv') # Table 3.2
...
abline(v = MI$statistic, col = 'red') # Figure 3.3
